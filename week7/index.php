...
<div class="col-lg-12">
    <table class="table table-bordered">
        ...
        <?php
        if (!empty($link)) {
            $res=mysqli_query($link,"select * from week7");
        }
        while($row=mysqli_fetch_array($res))
        {
            ...
            echo "<td>"; ?> <a href="delete.php?id=<?php echo $row["id"]; ?>" onclick="return confirm('Bạn có chắc chắn muốn xóa không?');"><button type="button" class="btn btn-danger">Delete </button></a> <?php echo "</td>";
            echo "</tr>";
        }
        ?>
        </tbody>
...
{
    mysqli_query($link,"insert into week7 values (NULL,'$_POST[firstname]' ,'$_POST[lastname]','$_POST[email]','$_POST[contact]')");
   ?>
...
{
    mysqli_query($link,"delete from week7 where firstname='$_POST[firstname]'");
    ?>
    <script type="text/javascript">
        window.location.href=window.location.href;
    </script>
...
{
    mysqli_query($link,"update week7 set firstname='$_POST[lastname]'where firstname='$_POST[firstname]'");
    ?>
    <script type="text/javascript">
        window.location.href=window.location.href;
    </script>
